pipeline {
    ...
    stages {
        ...
        stage('Build Docker Image') {
            ...
            steps {
                script {
                    app = docker.build("shahabuuu/react-app")
                    app.inside {
        ...
        stage('Push Docker Image') {
            ...
            steps {
                script {
                    docker.withRegistry('https://registry.hub.docker.com', 'shahabuddinkhan') {
         ...
         stage('DeployToStaging') {
            ...
            steps {
                    script {
                        sh "docker pull shahabuuu/react-app:${env.BUILD_NUMBER}"
                        try {
                        ...
                        sh "docker run --restart always --name react-app -p 1233:80 -d shahabuuu/react-app:${env.BUILD_NUMBER}"
                    }
            }
        }
